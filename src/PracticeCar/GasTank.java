...
public class GasTank extends CarPart {
    ...
    public void fillTank(float numOfGallons) {
        ...
        if (this.condition < 0) {
            this.status("Nice try, but you can't steal my gas with a negative" +
                    " number! Adding " + remainder + " gallons to reach capacity.");
            this.setCondition(this.bestCondition);
        } else if (numOfGallons > remainder) {
            this.status("Too much! Only adding " + remainder + " gallons to " +
                    "reach capacity.");
            this.setCondition(this.bestCondition);
        } else {
